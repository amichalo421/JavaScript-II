...
//PROCESS
// create function that creates a new array from runners(see jsI notes)
// use ".foreach" to make a function that makes a new array with runners first name and last name

// Create const formula "fullNames"
// Create an array to push the first and last name values to
// runners.foreach push first name and last name
...
//HELPGROUPSOLUTION (Technically not a perfect answer (Does not use ".foreach")):
...
// Create a function
// Create an array to contain the values that will be logged by function
// Create a formula to push to that array
    // That formula needs to use .map on runners
    // ??? Make it go to uppercase?
const firstNameAllCaps = (id) => {
  let firstNamesContainer = [];
  let runners = runners.map(id => return (first_name.touppercase()));
  firstNamesContainer.push(runners);
...
// //HELP GROUP SOLUTION (It checks out!):
...
// containing only those runners with large sized shirts so they can choose a different size. 
...
// runners.filter((i) => {
//   runnersLargeSizeShirt.push(runners.filter(runners.shirt_size === L));
// });
...
// // ???? How would you do an array of just names for those individuals with no other details. 
// // ???? How would you do an array of names no strings.
// // ???? How would you do an array of those names with strings.
// // ???? I just realized that I may be getting errors because my arrays are using const...could that be true?


// // HELP GROUP SOLUTION...(Doesn't check out...):
// let runnersLargeSizeShirt = runners.filter(eachrunner => {
//   let runners.shirt_size === L; 
// });
// console.log(runnersLargeSizeShirt);
...
const reducedDonations = runners.reduce((total, runner) => {
...
// // MY ANSWER:
// let ticketPriceTotal = runners.reduce(eachrunner => {
//   return runners.donation
// });

// //SOLUTION:
// let ticketPriceTotal = [];
// let reducetickets = runners.reduce((sum, value) => {
//   return sum + value['donation']
// }, 0)
// ticketPriceTotal.push(reducetickets);
// console.log(ticketPriceTotal);

// // ???? WHAT'S THE DEAL WITH SUM HERE AS A VARIABLE...WHY CAN'T YOU JUST ADD ALL OF THE DONATION VALUES.
// // ???? WHAT'S THE DEAL WITH THE ZERO AT THE END I STILL DON'T UNDERSTAND THAT ENTIRELY...
